...
def play_audio(video_file):
    audioclip = VideoFileClip(video_file).audio
    videogclip = VideoFileClip(video_file).set_audio(audioclip)
    audioclip.preview()
...
def fun(name):
    ...
    _thread = threading.Thread(target=play_audio, args=(new_name,))
    _thread.daemon = True
    _thread.start()
    ...
    while cap.isOpened():
        ...
        if cv2.getWindowProperty('image', 0) < 0 or cv2.waitKey(7) & 0xFF == ord('q'):
            break
